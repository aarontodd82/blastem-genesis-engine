...
# Always check uname for MSYS2/MINGW detection (environment $OS may be Windows_NT)
...
ifdef MINGW_BUILD
# MSYS2/MINGW64 native build - use pkg-config for SDL2/GLEW, link Windows GL directly
MEM:=mem_win.o
...
EXE:=.exe
SO:=dll
# Don't use pkg-config --cflags for SDL2 as it adds -Dmain=SDL_main which breaks non-SDL tools
# Just use the include path directly
# Note: -Wno-incompatible-pointer-types needed for newer GCC with BlastEm's code
CFLAGS:=-std=gnu99 -Wreturn-type -Werror=return-type -Werror=implicit-function-declaration -Wpointer-arith -Wno-incompatible-pointer-types
CFLAGS+= -I/mingw64/include/SDL2 $(shell pkg-config --cflags glew)
LDFLAGS:=-lm -lws2_32 -lmingw32 -lSDL2main -lSDL2
LDFLAGS+= $(shell pkg-config --libs glew) -lopengl32
...
AUDIOOBJS=ym2612.o psg.o wave.o vgm.o event_log.o render_audio.o serial_bridge.o
